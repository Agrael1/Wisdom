...
//-----------------------------------------------------------------------------
WIS_EXTERN_C WISDOM_API WisResult wisVKPipelineCacheSerialize(const WisVKPipelineCache* self,
                                                              uint8_t*                  data,
                                                              size_t                    data_size)
{
...
WIS_EXTERN_C WISDOM_API size_t wisVKPipelineCacheGetSerializedSize(const WisVKPipelineCache* self)
{
    ...
    return wis::aligned_size(data_size, 4096u); // Align to 4096 bytes for better memory management when this data is used to create a new pipeline cache
}
