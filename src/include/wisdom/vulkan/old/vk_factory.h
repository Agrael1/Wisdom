...
public:
    ...
    static wis::VKMainGlobal& GetGlobalTable() noexcept { return Instance().global_table; }
...
public:
    [[nodiscard]] WIS_INLINE VKAdapter GetAdapter(
        wis::Result& resutlt,
        uint32_t index,
        AdapterPreference preference = AdapterPreference::Performance
    ) const noexcept;

public:
    WIS_INLINE VkResult VKEnumeratePhysicalDevices() noexcept;

    [[nodiscard]] static WIS_INLINE wis::detail::fixed_allocation<const char*> FoundExtensions(
        wis::Result& result,
        std::span<const char*> in_extensions
    ) noexcept;
    [[nodiscard]] static WIS_INLINE wis::detail::fixed_allocation<const char*> FoundLayers(
        wis::Result& result,
        std::span<const char*> in_layers
    ) noexcept;
...
public:
     ...
     * */
    [[nodiscard]] inline wis::VKAdapter GetAdapter(
        wis::Result& result,
        uint32_t index,
        wis::AdapterPreference preference = wis::AdapterPreference::Performance
    ) const noexcept
    {
     ...
     * @return wis::VKAdapter on success (wis::Status::Ok).
     * */
    [[nodiscard]] inline wis::ResultValue<wis::VKAdapter> GetAdapter(
        uint32_t index,
        wis::AdapterPreference preference = wis::AdapterPreference::Performance
    ) const noexcept
    {
...
[[nodiscard]] WIS_INLINE wis::VKFactory ImplVKCreateFactory(
    wis::Result& result,
    bool debug_layer,
    VKFactoryExtension** extensions,
