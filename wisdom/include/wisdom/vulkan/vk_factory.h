...
public:
    ...
    /// @param unused
    WIS_INLINE VKFactory(const ApplicationInfo& app_info = {});
...
private:
    ...
    {
        auto vextensions = vk::enumerateInstanceExtensionProperties().value;
        for (auto& i : vextensions)
            extensions.emplace(i.extensionName, i);
    }
    void LoadLayers() noexcept
    {
        auto vlayers = vk::enumerateInstanceLayerProperties().value;
        for (auto& i : vlayers)
...
wis::VKFactory::VKFactory(const ApplicationInfo& app_info)
{
