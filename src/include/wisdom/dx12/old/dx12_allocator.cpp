...
wis::DX12Buffer wis::ImplDX12ResourceAllocator::CreateBuffer(
    wis::Result& result,
    uint64_t size,
...
) const noexcept
{
    ...
    D3D12MA::ALLOCATION_DESC all_desc{
        .Flags = wis::detail::DX12Convert(mem_flags),
        .HeapType = wis::detail::DX12Convert(memory),
    };
    if (usage & wis::BufferUsage::AccelerationStructureBuffer) {
        return DX12CreateResource2(
            result,
            all_desc,
            buffer_desc,
...
wis::DX12Texture wis::ImplDX12ResourceAllocator::CreateTexture(
    wis::Result& result,
...
wis::AllocationInfo wis::ImplDX12ResourceAllocator::GetTextureAllocationInfo(
    const wis::TextureDesc& desc
) const noexcept
{
    ...
    return AllocationInfo{.size_bytes = info.SizeInBytes, .alignment_bytes = info.Alignment};
}
wis::AllocationInfo wis::ImplDX12ResourceAllocator::GetBufferAllocationInfo(
    uint64_t size,
    BufferUsage flags
) const noexcept
{
...
wis::DX12Memory wis::ImplDX12ResourceAllocator::AllocateTextureMemory(
    wis::Result& result,
    uint64_t size,
...

{
    ...
    if ((mem_flags & MemoryFlags::Exportable)) {
        if (memory != MemoryType::Default) {
            result = wis::make_result<wis::Func<wis::FuncD()>(), "Exportable memory must be Default heap type">(
                E_INVALIDARG
            );
            return out_memory;
        }
    ...
    auto info = GetTextureAllocationInfo(
        {.format = wis::DataFormat::RGBA8Unorm, .size = {1, 1, 1}, .mip_levels = 1, .usage = usage}
    );
...
wis::DX12Memory wis::ImplDX12ResourceAllocator::AllocateBufferMemory(
    wis::Result& result,
    uint64_t size,
    wis::BufferUsage usage,
    wis::MemoryType memory,
...
wis::DX12Buffer wis::ImplDX12ResourceAllocator::PlaceBuffer(
    wis::Result& result,
...
) const noexcept
{
    ...
    auto hr = allocator->CreateAliasingResource1(
        alloc,
        memory_offset,
        &buffer_desc,
        D3D12_RESOURCE_STATE_COMMON,
        nullptr,
        internal.resource.iid(),
        internal.resource.put_void()
    );
...
wis::DX12Texture wis::ImplDX12ResourceAllocator::PlaceTexture(
    wis::Result& result,
    DX12MemoryView memory,
    uint64_t memory_offset,
    const wis::TextureDesc& desc
...
wis::DX12Buffer wis::ImplDX12ResourceAllocator::DX12CreateResource(
    wis::Result& result,
...
) const noexcept
{
    ...
    HRESULT hr = allocator->CreateResource2(
        ...
        state,
        nullptr,
        memory_internal.allocation.put(),
        __uuidof(*internal.resource),
        internal.resource.put_void()
    );
...
wis::DX12Buffer wis::ImplDX12ResourceAllocator::DX12CreateResource2(
    wis::Result& result,
    const D3D12MA::ALLOCATION_DESC& all_desc,
    const D3D12_RESOURCE_DESC1& res_desc,
    D3D12_RESOURCE_STATES state
...
void wis::ImplDX12ResourceAllocator::DX12FillBufferDesc(
    uint64_t size,
    BufferUsage flags,
    D3D12_RESOURCE_DESC1& info
) noexcept
{
    ...
    info = CD3DX12_RESOURCE_DESC1::Buffer(size, wis::detail::DX12Convert(flags));
}
void wis::ImplDX12ResourceAllocator::DX12FillTextureDesc(const TextureDesc& desc, D3D12_RESOURCE_DESC1& info) noexcept
{
    ...
    case wis::TextureLayout::Texture1D:
        info = CD3DX12_RESOURCE_DESC1::Tex1D(
            ...
            wis::detail::DX12Convert(desc.usage)
        );
        break;
    default:
    ...
    case wis::TextureLayout::Texture2DMS:
        info = CD3DX12_RESOURCE_DESC1::Tex2D(
            ...
            uint16_t(1),
            uint16_t(1),
            wis::detail::DX12Convert(desc.sample_count),
            4,
            wis::detail::DX12Convert(desc.usage)
        );
        break;
