...
wis::VKBuffer wis::ImplVKResourceAllocator::CreateBuffer(
    wis::Result& result,
    uint64_t size,
    wis::BufferUsage usage,
    wis::MemoryType memory,
    wis::MemoryFlags mem_flags
) const noexcept
{
    VkBufferCreateInfo desc;
    VKFillBufferDesc(size, usage, desc);

    VmaAllocationCreateFlags flags = wis::wis::detail::VKConvert(mem_flags);
    ...
    VmaAllocationCreateInfo alloc{
        ...
        .requiredFlags = wis::wis::detail::VKConvert(memory)
    };
    return VKCreateBuffer(result, desc, alloc);
}

wis::VKTexture wis::ImplVKResourceAllocator::CreateTexture(
    wis::Result& result,
...
) const noexcept
{
    ...
    VmaAllocationCreateInfo alloc{
        .flags = wis::wis::detail::VKConvert(mem_flags) & ~VMA_ALLOCATION_CREATE_MAPPED_BIT,
        .usage = VmaMemoryUsage::VMA_MEMORY_USAGE_AUTO,
        .requiredFlags = wis::wis::detail::VKConvert(memory)
    };
...
wis::AllocationInfo wis::ImplVKResourceAllocator::GetTextureAllocationInfo(const wis::TextureDesc& desc) const noexcept
{
...
wis::AllocationInfo wis::ImplVKResourceAllocator::GetBufferAllocationInfo(
    uint64_t size,
    BufferUsage flags
) const noexcept
{
...
wis::VKMemory wis::ImplVKResourceAllocator::AllocateTextureMemory(
    wis::Result& result,
    uint64_t size,
...
) const noexcept
{
    ...
    VmaAllocationCreateInfo alloc_desc{
        .flags = wis::detail::VKConvert(mem_flags),
        .usage = vma_usage,
        .requiredFlags = wis::wis::detail::VKConvert(memory),
    };
...
wis::VKMemory wis::ImplVKResourceAllocator::AllocateBufferMemory(
    wis::Result& result,
    uint64_t size,
    wis::BufferUsage usage,
    wis::MemoryType memory,
...
wis::VKBuffer wis::ImplVKResourceAllocator::PlaceBuffer(
    wis::Result& result,
...
wis::VKTexture wis::ImplVKResourceAllocator::PlaceTexture(
    wis::Result& result,
    wis::VKMemoryView memory,
    uint64_t memory_offset,
    const wis::TextureDesc& desc
...
wis::VKTexture wis::ImplVKResourceAllocator::VKCreateTexture(
    wis::Result& result,
...
wis::VKBuffer wis::ImplVKResourceAllocator::VKCreateBuffer(
    wis::Result& result,
    VkBufferCreateInfo& desc,
    const VmaAllocationCreateInfo& alloc_desc,
    bool interop
...
wis::VKBuffer wis::ImplVKResourceAllocator::VKCreateAliasingBuffer(
    wis::Result& result,
    VkBufferCreateInfo& desc,
    VkDeviceSize offset,
    VmaAllocation alloc,
...
wis::VKTexture wis::ImplVKResourceAllocator::VKCreateAliasingTexture(
    wis::Result& result,
    VkImageCreateInfo& desc,
    VkDeviceSize offset,
    VmaAllocation alloc,
...
void wis::ImplVKResourceAllocator::VKFillBufferDesc(
    uint64_t size,
...
) noexcept
{
    info = {
        ...
        .size = size,
        .usage = VkBufferUsageFlags(VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | wis::detail::VKConvert(flags)),
    };
}
void wis::ImplVKResourceAllocator::VKFillImageDesc(const wis::TextureDesc& desc, VkImageCreateInfo& info) noexcept
{
    info = {
        ...
        .usage = wis::detail::VKConvert(desc.usage),
...
void wis::ImplVKResourceAllocator::VKFillTextureAllocationInfo(
    const wis::TextureDesc& desc,
...
) const noexcept
{
    ...
    dtable.vkGetDeviceImageMemoryRequirements(allocator.header().get(), &devImgMemReq, &memReq);
    out_info = memReq;
}
void wis::ImplVKResourceAllocator::VKFillBufferAllocationInfo(
    uint64_t size,
    wis::BufferUsage flags,
    VkMemoryRequirements2& out_info
) const noexcept
{
    ...
    VkMemoryRequirements2 memReq{.sType = VK_STRUCTURE_TYPE_MEMORY_REQUIREMENTS_2};
    auto& dtable = allocator.header().table();
    dtable.vkGetDeviceBufferMemoryRequirements(allocator.header().get(), &devImgMemReq, &memReq);
    out_info = memReq;
}
