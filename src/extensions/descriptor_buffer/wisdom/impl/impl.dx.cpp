...
wis::ImplDX12DescriptorBufferExtension::CreateRootSignature(wis::Result& result, const PushConstant* root_constants, uint32_t constants_size, const PushDescriptor* push_descriptors, uint32_t push_descriptors_size, const wis::DescriptorTable* tables, uint32_t tables_count) const noexcept
{
    ...
    hr = device->CreateRootSignature(0, signature->GetBufferPointer(), signature->GetBufferSize(), __uuidof(*internal.root), internal.root.put_void());
    if (!wis::succeeded(hr)) {
...
wis::ImplDX12DescriptorBufferExtension::CreateDescriptorBuffer(wis::Result& result, wis::DescriptorHeapType heap_type, wis::DescriptorMemory memory_type, uint64_t memory_bytes) const noexcept
{
