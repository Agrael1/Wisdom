...
wis::ImplDX12CreateDevice(wis::Result& result, wis::DX12Adapter adapter, wis::DX12DeviceExtension** extensions, uint32_t ext_count, bool force) noexcept
{
    ...
    HRESULT hr = D3D12CreateDevice(internal.adapter.get(), D3D_FEATURE_LEVEL_11_0, __uuidof(ID3D12Device9), internal.device.put_void());
...
wis::ImplDX12Device::CreateFence(wis::Result& result, uint64_t initial_value, wis::FenceFlags flags) const noexcept
{
    ...
    HRESULT hr = device->CreateFence(initial_value, convert_dx(flags), internal.fence.iid(), internal.fence.put_void());
...
wis::ImplDX12Device::CreateCommandList(wis::Result& result, wis::QueueType type) const noexcept
{
    ...
    hr = device->CreateCommandList(0, clty, internal.allocator.get(), nullptr, internal.list.iid(), internal.list.put_void());
...
wis::DX12Shader
wis::ImplDX12Device::CreateShader(wis::Result& result, void* data, size_t size) const noexcept
{
                                         ...
                                         uint32_t                          descriptor_bindings_count) const noexcept
{
    ...
    hr = device->CreateRootSignature(0, signature->GetBufferPointer(), signature->GetBufferSize(), internal.root.iid(), internal.root.put_void());
