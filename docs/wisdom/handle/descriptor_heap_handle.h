/**
 * @struct WisDescriptorHeap
 * @ingroup Handles
 *
 *
 * @section WisDescriptorHeap_spec Specification
 * <hr>
 *
 * \cond WIS_GEN_CODE
 * Vulkan Version:
 * ```c
 * // Provided by Wisdom 0.7.0. 
 * WIS_DEFINE_HANDLE(WisVKDescriptorHeap,7);
 * ```
 * DX12 Version:
 * ```c
 * // Provided by Wisdom 0.7.0. 
 * WIS_DEFINE_HANDLE(WisDX12DescriptorHeap,2);
 * ```
 * \endcond
 *
 * @section WisDescriptorHeap_see_also See Also
 * <hr>
 * \cond WIS_GEN_REFS
 * @see wisDestroyDescriptorHeap, wisDeviceCreateDescriptorHeap, wisDescriptorHeapGetCPUHandle, wisCommandListSetDescriptorHeaps, wisCommandListSetDescriptorHeaps
 * \endcond
 */