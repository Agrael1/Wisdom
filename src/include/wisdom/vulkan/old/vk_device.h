...
public:
    auto& GetInstanceTable() const noexcept { return adapter.GetInternal().instance.table(); }
    auto& GetDeviceTable() const noexcept { return device.table(); }
...
{
    friend wis::VKDevice ImplVKCreateDevice(
        wis::Result& result,
...
public:
    WIS_INLINE wis::Result WaitForMultipleFences(
    ...
    [[nodiscard]] WIS_INLINE wis::VKFence CreateFence(
        wis::Result& result,
        uint64_t initial_value = 0ull,
        wis::FenceFlags flags = wis::FenceFlags::None
    ) const noexcept;

    [[nodiscard]] WIS_INLINE wis::VKCommandQueue CreateCommandQueue(
        wis::Result& result,
        wis::QueueType type
    ) const noexcept;

    [[nodiscard]] WIS_INLINE wis::VKCommandList CreateCommandList(
        wis::Result& result,
        wis::QueueType type
    ) const noexcept;

    [[nodiscard]] WIS_INLINE wis::VKPipelineState CreateGraphicsPipeline(
        wis::Result& result,
        const wis::VKGraphicsPipelineDesc& desc
    ) const noexcept;

    [[nodiscard]] WIS_INLINE wis::VKPipelineState CreateComputePipeline(
        wis::Result& result,
        const wis::VKComputePipelineDesc& desc
    ) const noexcept;

    [[nodiscard]] WIS_INLINE wis::VKShader CreateShader(
        wis::Result& result,
        void* bytecode,
        uint32_t size
    ) const noexcept;

    [[nodiscard]] WIS_INLINE wis::VKResourceAllocator CreateAllocator(wis::Result& result) const noexcept;

    [[nodiscard]] WIS_INLINE wis::VKRenderTarget CreateRenderTarget(
        wis::Result& result,
        VKTextureView texture,
        wis::RenderTargetDesc desc
    ) const noexcept;

    [[nodiscard]] WIS_INLINE wis::VKRenderTarget CreateDepthStencilTarget(
        wis::Result& result,
        VKTextureView texture,
        wis::RenderTargetDesc desc
    ) const noexcept
    ...
    [[nodiscard]] WIS_INLINE wis::VKSampler CreateSampler(
        wis::Result& result,
        const wis::SamplerDesc& desc
    ) const noexcept;

    [[nodiscard]] WIS_INLINE wis::VKShaderResource CreateShaderResource(
        wis::Result& result,
        wis::VKTextureView texture,
        const wis::ShaderResourceDesc& desc
    ) const noexcept;

    [[nodiscard]] WIS_INLINE wis::VKUnorderedAccessTexture CreateUnorderedAccessTexture(
        wis::Result& result,
        wis::VKTextureView texture,
        const wis::UnorderedAccessDesc& desc
    ) const noexcept
    {
    ...
    [[nodiscard]] WIS_INLINE bool QueryFeatureSupport(wis::DeviceFeature feature) const noexcept;

    [[nodiscard]] WIS_INLINE wis::VKDescriptorStorage CreateDescriptorStorage(
        wis::Result& result,
        const wis::DescriptorBindingDesc* descriptor_bindings = nullptr,
        uint32_t descriptor_bindings_count = 0,
        wis::DescriptorMemory = wis::DescriptorMemory::ShaderVisible
    ) const noexcept;

    [[nodiscard]] WIS_INLINE wis::VKRootSignature CreateRootSignature(
        wis::Result& result,
    ...
    [[nodiscard]] WIS_INLINE wis::DeviceConstants QueryDeviceConsts() const noexcept;

public:
    [[nodiscard]] WIS_INLINE wis::VKSwapChain VKCreateSwapChain(
        wis::Result& result,
...
protected:
    [[nodiscard]] WIS_INLINE wis::shared_handle<VmaAllocator> VKCreateAllocator(
        wis::Result& result,
        bool interop = false
    ) const noexcept;
};
...
public:
     ...
     * */
    [[nodiscard]] inline wis::Result WaitForMultipleFences(
        const wis::VKFenceView* fences,
        const uint64_t* fence_values,
        uint32_t fence_count,
        wis::MutiWaitFlags wait_all = wis::MutiWaitFlags::All,
     ...
     * */
    [[nodiscard]] inline wis::VKFence CreateFence(
        wis::Result& result,
     ...
     * @return wis::VKFence on success (wis::Status::Ok).
     * */
    [[nodiscard]] inline wis::ResultValue<wis::VKFence> CreateFence(
        uint64_t initial_value = 0,
        wis::FenceFlags flags = wis::FenceFlags::None
    ) const noexcept
    {
     ...
     * */
    [[nodiscard]] inline wis::VKPipelineState CreateGraphicsPipeline(
        wis::Result& result,
        const wis::VKGraphicsPipelineDesc& desc
    ) const noexcept
    {
     ...
     * @return wis::VKPipelineState on success (wis::Status::Ok).
     * */
    [[nodiscard]] inline wis::ResultValue<wis::VKPipelineState> CreateGraphicsPipeline(
        const wis::VKGraphicsPipelineDesc& desc
    ) const noexcept
    {
     ...
     * @return wis::VKPipelineState on success (wis::Status::Ok).
     * */
    [[nodiscard]] inline wis::VKPipelineState CreateComputePipeline(
        wis::Result& result,
        const wis::VKComputePipelineDesc& desc
    ) const noexcept
    {
     ...
     * @return wis::VKPipelineState on success (wis::Status::Ok).
     * */
    [[nodiscard]] inline wis::ResultValue<wis::VKPipelineState> CreateComputePipeline(
        const wis::VKComputePipelineDesc& desc
    ) const noexcept
    {
     ...
     * DescriptorStorage is used for bindless and non-uniform bindings. Don't combine with Descriptor buffers, this may
     * reduce performance. Push constants and push descriptors are used for fast changing data. Spaces may not overlap,
     * but can be in any order. Push descriptors always have space0 and [[vk::binding(x,0)]]. That means that all the
     * binding numbers are off by 1. Meaning that if you have Descriptor Storage with 1 binding, it will be
     * [[vk::binding(0,1)]] even though it is supposed to be binding 0. This is done for consistency. Set number is the
     * position of binding in bindings array. e.g. bindings[5] is set 5 and on HLSL side it is [[vk::binding(0,5)]]. For
     * several overlapping types e.g. 2D and 3D textures, use different spaces. Those are specified in the bindings
     * array. Space overlap count means how many consecutive spaces are used by the binding.
     ...
     * In shader will appear in order of submission. e.g. push_descriptors[5] is [[vk::binding(5,0)]] ... :
     * register(b5/t5/u5)
     ...
     * One block of bindings can contain up to 4096 descriptors. For Sampler blocks, max amount of samplers across all
     * bindings is 2048.
     ...
     * */
    [[nodiscard]] inline wis::VKRootSignature CreateRootSignature(
        wis::Result& result,
        const wis::PushConstant* push_constants = nullptr,
        uint32_t push_constant_count = 0,
        const wis::PushDescriptor* push_descriptors = nullptr,
        uint32_t push_descriptor_count = 0,
        const wis::DescriptorBindingDesc* bindings = nullptr,
        uint32_t binding_count = 0
    ) const noexcept
    {
        return wis::ImplVKDevice::CreateRootSignature(
            result,
            push_constants,
            push_constant_count,
            push_descriptors,
     ...
     * @return wis::VKRootSignature on success (wis::Status::Ok).
     * */
    [[nodiscard]] inline wis::ResultValue<wis::VKRootSignature> CreateRootSignature(
        const wis::PushConstant* push_constants = nullptr,
        uint32_t push_constant_count = 0,
        const wis::PushDescriptor* push_descriptors = nullptr,
        uint32_t push_descriptor_count = 0,
    ...
    {
        return wis::ResultValue<wis::VKRootSignature>{
            &wis::ImplVKDevice::CreateRootSignature,
            this,
            push_constants,
            push_constant_count,
            push_descriptors,
     ...
     * */
    [[nodiscard]] inline wis::VKRenderTarget CreateRenderTarget(
        wis::Result& result,
        wis::VKTextureView texture,
        const wis::RenderTargetDesc& desc
    ) const noexcept
    {
     ...
     * @return wis::VKRenderTarget on success (wis::Status::Ok).
     * */
    [[nodiscard]] inline wis::ResultValue<wis::VKRenderTarget> CreateRenderTarget(
        wis::VKTextureView texture,
        const wis::RenderTargetDesc& desc
    ) const noexcept
    {
        return wis::ResultValue<wis::VKRenderTarget>{
            &wis::ImplVKDevice::CreateRenderTarget,
            this,
            std::move(texture),
            desc
        };
     ...
     * @return wis::VKRenderTarget on success (wis::Status::Ok).
     * */
    [[nodiscard]] inline wis::VKRenderTarget CreateDepthStencilTarget(
        wis::Result& result,
        wis::VKTextureView texture,
        const wis::RenderTargetDesc& desc
    ) const noexcept
     ...
     * @return wis::VKRenderTarget on success (wis::Status::Ok).
     * */
    [[nodiscard]] inline wis::ResultValue<wis::VKRenderTarget> CreateDepthStencilTarget(
        wis::VKTextureView texture,
        const wis::RenderTargetDesc& desc
    ) const noexcept
    {
        return wis::ResultValue<wis::VKRenderTarget>{
            &wis::ImplVKDevice::CreateDepthStencilTarget,
            this,
            std::move(texture),
            desc
        };
     ...
     * */
    [[nodiscard]] inline wis::VKShaderResource CreateShaderResource(
        wis::Result& result,
        wis::VKTextureView texture,
        const wis::ShaderResourceDesc& desc
    ) const noexcept
     ...
     * @return wis::VKShaderResource on success (wis::Status::Ok).
     * */
    [[nodiscard]] inline wis::ResultValue<wis::VKShaderResource> CreateShaderResource(
        wis::VKTextureView texture,
        const wis::ShaderResourceDesc& desc
    ) const noexcept
    {
        return wis::ResultValue<wis::VKShaderResource>{
            &wis::ImplVKDevice::CreateShaderResource,
            this,
            std::move(texture),
            desc
        };
     ...
     * */
    [[nodiscard]] inline wis::VKDescriptorStorage CreateDescriptorStorage(
        wis::Result& result,
        const wis::DescriptorBindingDesc* bindings,
        uint32_t bindings_count,
        wis::DescriptorMemory memory = wis::DescriptorMemory::ShaderVisible
    ) const noexcept
    {
     ...
     * @return wis::VKDescriptorStorage on success (wis::Status::Ok).
     * */
    [[nodiscard]] inline wis::ResultValue<wis::VKDescriptorStorage> CreateDescriptorStorage(
        const wis::DescriptorBindingDesc* bindings,
        uint32_t bindings_count,
        wis::DescriptorMemory memory = wis::DescriptorMemory::ShaderVisible
    ) const noexcept
    {
        return wis::ResultValue<wis::VKDescriptorStorage>{
            &wis::ImplVKDevice::CreateDescriptorStorage,
            this,
            bindings,
            bindings_count,
            memory
     ...
     * */
    inline wis::DeviceConstants QueryDeviceConsts() const noexcept { return wis::ImplVKDevice::QueryDeviceConsts(); }
};
#pragma endregion VKDevice

[[nodiscard]] WIS_INLINE wis::VKDevice ImplVKCreateDevice(
    wis::Result& result,
    wis::VKAdapter in_adapter,
    wis::VKDeviceExtension** exts,
    uint32_t ext_size,
