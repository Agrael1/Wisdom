...
wis::DX12Device wis::ImplDX12CreateDevice(
    wis::Result& result,
...
) noexcept
{
    ...
    HRESULT hr = D3D12CreateDevice(
        internal.adapter.get(),
        D3D_FEATURE_LEVEL_11_0,
        __uuidof(ID3D12Device9),
        internal.device.put_void()
    );
...
wis::Result wis::ImplDX12Device::WaitForMultipleFences(
...
) const noexcept
{
    ...
    if (!wis::detail::succeeded(
            hr = device->SetEventOnMultipleFenceCompletion(
    ...
    {
        return wis::make_result<
            wis::Func<wis::FuncD()>(),
            "ID3D12Device10::SetEventOnMultipleFenceCompletion failed to set "
...
wis::DX12Fence wis::ImplDX12Device::CreateFence(
    wis::Result& result,
...
) const noexcept
{
    ...
    HRESULT hr = device->CreateFence(
        initial_value,
        wis::detail::DX12Convert(flags),
        internal.fence.iid(),
        internal.fence.put_void()
    );
...
wis::DX12CommandQueue wis::ImplDX12Device::CreateCommandQueue(wis::Result& result, wis::QueueType type) const noexcept
{
    ...
    if (!wis::succeeded(hr)) {
        result = wis::make_result<
            wis::Func<wis::FuncD()>(),
            "ID3D12Device10::CreateCommandQueue failed to create command queue">(hr);
    }

    return out_queue;
}

wis::DX12CommandList wis::ImplDX12Device::CreateCommandList(wis::Result& result, wis::QueueType type) const noexcept
{
    ...
    hr = device->CreateCommandList(
        0,
        clty,
        internal.allocator.get(),
        nullptr,
        internal.list.iid(),
        internal.list.put_void()
    );
...
wis::DX12PipelineState wis::ImplDX12Device::CreateGraphicsPipeline(
    wis::Result& result,
...
) const noexcept
{
    ...
    //--Topology
    pipeline_stream.allocate<CD3DX12_PIPELINE_STATE_STREAM_PRIMITIVE_TOPOLOGY>() = wis::detail::DX12Convert(
        desc.topology_type
    );
    ...
    auto slots = std::span{desc.input_layout.slots, desc.input_layout.slots + desc.input_layout.slot_count};
    auto attrs = std::span{
        desc.input_layout.attributes,
    ...
    if (desc.rasterizer) {
        ...
        pipeline_stream.allocate<CD3DX12_PIPELINE_STATE_STREAM_RASTERIZER2>() = CD3DX12_RASTERIZER_DESC2{
    ...
    if (desc.depth_stencil) {
        ...
        pipeline_stream.allocate<CD3DX12_PIPELINE_STATE_STREAM_DEPTH_STENCIL2>() = CD3DX12_DEPTH_STENCIL_DESC2{
...
wis::DX12PipelineState wis::ImplDX12Device::CreateComputePipeline(
    wis::Result& result,
...
) const noexcept
{
    ...
    //--Shader stages
    wis::detail::memory_pool pipeline_stream;
    wis::detail::DX12FillShaderStage<CD3DX12_PIPELINE_STATE_STREAM_CS>(pipeline_stream, desc.shader);
...
wis::DX12Shader wis::ImplDX12Device::CreateShader(wis::Result& result, void* data, size_t size) const noexcept
{
    ...
    if (!internal.bytecode) {
        result = wis::make_result<wis::Func<wis::FuncD()>(), "Failed to allocate memory for shader bytecode">(
            E_OUTOFMEMORY
        );
        return out_shader;
    }
...
wis::DX12ResourceAllocator wis::ImplDX12Device::CreateAllocator(wis::Result& result) const noexcept
{
...
wis::DX12RenderTarget wis::ImplDX12Device::CreateRenderTarget(
    wis::Result& result,
...
) const noexcept
{
    ...
    case wis::TextureLayout::Texture1D:
        rtv_desc.Texture1D = {.MipSlice = desc.mip};
        break;
    ...
    case wis::TextureLayout::Texture2D:
        rtv_desc.Texture2D = {.MipSlice = desc.mip};
        break;
    case wis::TextureLayout::Texture2DMSArray:
        rtv_desc.Texture2DMSArray = {.FirstArraySlice = desc.base_array_layer, .ArraySize = desc.layer_count};
        break;
    case wis::TextureLayout::Texture3D:
        rtv_desc.Texture3D = {.MipSlice = desc.mip, .FirstWSlice = desc.base_array_layer, .WSize = desc.layer_count};
        break;
    default:
        break;
    }
    ...
    if (!wis::succeeded(hr)) {
        result = wis::make_result<wis::Func<wis::FuncD()>(), "Failed to create descriptor heap for render target view">(
            hr
        );
        return out_target;
    }
...
wis::DX12RenderTarget wis::ImplDX12Device::CreateDepthStencilTarget(
    wis::Result& result,
    DX12TextureView texture,
    wis::RenderTargetDesc desc
) const noexcept
{
    ...
    D3D12_DEPTH_STENCIL_VIEW_DESC rtv_desc{
        ...
        .Texture2DArray{.MipSlice = desc.mip, .FirstArraySlice = desc.base_array_layer, .ArraySize = desc.layer_count}
    };
    switch (desc.layout) {
    case wis::TextureLayout::Texture1D:
        rtv_desc.Texture1D = {.MipSlice = desc.mip};
...
wis::DX12Sampler wis::ImplDX12Device::CreateSampler(wis::Result& result, const wis::SamplerDesc& desc) const noexcept
{
    ...
    auto basic_filter = D3D12_ENCODE_BASIC_FILTER(
        min_filter,
        mag_filter,
        wis::detail::DX12Convert(desc.mip_filter),
...
wis::DX12ShaderResource wis::ImplDX12Device::CreateShaderResource(
    wis::Result& result,
    DX12TextureView texture,
...
) const noexcept
{
    ...
    device->CreateShaderResourceView(
        std::get<0>(texture),
        &srv_desc,
        internal.heap->GetCPUDescriptorHandleForHeapStart()
    );
    return out_resource;
}

wis::DX12UnorderedAccessTexture wis::ImplDX12Device::CreateUnorderedAccessTexture(
    wis::Result& result,
    DX12TextureView texture,
...
wis::DX12DescriptorStorage wis::ImplDX12Device::CreateDescriptorStorage(
    wis::Result& result,
    const wis::DescriptorBindingDesc* descriptor_bindings,
    uint32_t descriptor_bindings_count,
    wis::DescriptorMemory memory
) const noexcept
{
    ...
    if (size_resources) {
        ...
        auto hr = device->CreateDescriptorHeap(
            &resource_heap_desc,
            internal.heaps[0].iid(),
            internal.heaps[0].put_void()
        );
        if (!wis::succeeded(hr)) {
            result = wis::make_result<wis::Func<wis::FuncD()>(), "Failed to create descriptor heap for resources">(hr);
            return out_storage;
        }
        internal.heap_resource_increment = device->GetDescriptorHandleIncrementSize(
    ...
    if (!internal.heap_offsets) {
        result = wis::make_result<wis::Func<wis::FuncD()>(), "Failed to allocate memory for descriptor offsets">(
            E_OUTOFMEMORY
        );
        return out_storage;
    }
...
wis::DX12RootSignature wis::ImplDX12Device::CreateRootSignature(
    wis::Result& result,
...
) const noexcept
{
    ...
    if (push_constants_count + push_descriptors_count * 2 + descriptor_bindings_count > 64) {
        result = wis::make_result<
            wis::Func<wis::FuncD()>(),
            "Root signature is only able to use 64 parameters, push descriptors count as 2 parameters">(E_INVALIDARG);
        return out_state;
    }
    ...
    std::unique_ptr<D3D12_DESCRIPTOR_RANGE1[]>
        memory = wis::detail::make_unique_for_overwrite<D3D12_DESCRIPTOR_RANGE1[]>(memory_size);
    if (!memory) {
        result = wis::make_result<wis::Func<wis::FuncD()>(), "Failed to allocate memory for descriptor ranges">(
            E_OUTOFMEMORY
        );
        return out_state;
    }
    ...
    desc.Init_1_1(
        push_constants_count + push_descriptors_count + descriptor_bindings_count,
        root_params,
        0,
        nullptr,
        flags
...
wis::DeviceConstants wis::ImplDX12Device::QueryDeviceConsts() const noexcept
{
