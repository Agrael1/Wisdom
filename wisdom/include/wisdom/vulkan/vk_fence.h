...
public:
    ...
    {
        auto [_, value] = device.getSemaphoreCounterValue(fence.get());
        return value;
    }
    ...
    bool Wait(uint64_t value) const noexcept // NOLINT
    {
        ...
        return wis::succeeded(device.waitSemaphores(waitInfo, std::numeric_limits<uint64_t>::max()));
    }
    ...
    void Signal(uint64_t value) const noexcept
    {
        ...
        wis::succeeded(device.signalSemaphore(&signalInfo));
    }
};
} // namespace wis
