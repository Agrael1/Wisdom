...
wis::DX12RaytracingPipeline wis::ImplDX12Raytracing::CreateRaytracingPipeline(
    wis::Result& result,
...
) const noexcept
{
    ...
    std::span<D3D12_EXPORT_DESC> export_span(
        reinterpret_cast<D3D12_EXPORT_DESC*>(dxil_library_span.data() + desc.shader_count),
        desc.export_count
    );
    std::span<D3D12_HIT_GROUP_DESC> hit_group_span(
        reinterpret_cast<D3D12_HIT_GROUP_DESC*>(export_span.data() + desc.export_count),
    ...
    wchar_t** callable_span = reinterpret_cast<wchar_t**>(
        subobjects.get() + string_offset
    ); // callable pointers in reverse order
    ...
    for (int32_t i = desc.export_count - 1; i >= 0; --i) {
        ...
        _export = {.Name = rename, .ExportToRename = name, .Flags = D3D12_EXPORT_FLAG_NONE};
    }
    ...
    for (uint32_t i = 0; i < desc.hit_group_count; ++i) {
        ...
        hit_group_span[i] = {
            .Type = wis::detail::DX12Convert(desc.hit_groups[i].type),
    ...
    // root signature
    D3D12_GLOBAL_ROOT_SIGNATURE global_root_signature{.pGlobalRootSignature = std::get<0>(desc.root_signature)};
    ...
    pipe_i.shader_identifiers = wis::detail::make_unique_for_overwrite<uint8_t[]>(
        (num_raygen + num_miss + num_callable + desc.hit_group_count) * D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES
    );
