...
/// @brief A DX12 device
WIS_EXPORT class DX12Device final : public QueryInternal<DX12Device>
{
public:
    DX12Device() = default;
    explicit DX12Device(DX12FactoryView factory, DX12AdapterView adapter) noexcept
    {
    ...
    WIS_INLINE bool
    Initialize(DX12FactoryView factory, DX12AdapterView adapter) noexcept;
...
public:
    ...
    [[nodiscard]] WIS_INLINE DX12CommandList
    CreateCommandList(QueueType list_type) const noexcept;

    /// @brief Create a fence
    [[nodiscard]] WIS_INLINE DX12Fence
    ...
    [[nodiscard]] DX12DescriptorHeap CreateDescriptorHeap(uint32_t num_descs, PoolType type) const
    {
        ...
        winrt::com_ptr<ID3D12DescriptorHeap> heap;
        wis::succeeded(device->CreateDescriptorHeap(&desc, __uuidof(*heap), heap.put_void()));
