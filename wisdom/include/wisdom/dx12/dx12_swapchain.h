...
public:
    ...
    explicit DX12SwapChain(winrt::com_ptr<IDXGISwapChain4> xchain, uint32_t frame_count, bool stereo) noexcept
        : QueryInternal(std::move(xchain)), stereo(stereo)
    {
        ...
        for (uint32_t n = 0; n < frame_count; n++) {
            ...
            if (!wis::succeded(chain->GetBuffer(n, __uuidof(ID3D12Resource), rc.put_void()))) {
                chain = nullptr;
...
public:
    ...
    [[nodiscard]] std::span<const DX12Texture> GetRenderTargets() const noexcept
    {
    ...
    [[nodiscard]] DX12TextureView GetBackBuffer() const noexcept
    {
    ...
    {
        return wis::succeded(chain->Present(0, 0));
    }
