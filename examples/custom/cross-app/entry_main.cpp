...
class App
{
    wis::platform::WindowsExtension platform;
...
public:
    App(HWND window)
    {
        ...
        auto swapx = platform.CreateSwapchain(result, setup.device, setup.queue, desc, window);
...
{
    try {
        auto file = fopen("C:\\Users\\Agrae\\source\\repos\\WpfApp1\\bin\\Debug\\Window.bin", "rb");
