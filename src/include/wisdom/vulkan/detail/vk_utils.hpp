 ...
 * @return A WisStatus value that corresponds to the given VkResult, providing a standardized status code for use within
 * the Wisdom library.
 */
 ...
 * on the source location of the call and the provided custom message.
 ...
 * @return A WisResult object that contains the converted WisStatus, original VkResult, and a descriptive error message
 ...
 * @tparam message A compile-time fixed string that provides additional context about the error. This message is
 * included in the generated error string to give more information about the failure.
 * @param status The WisStatus value that is custom, allowing the caller to specify a more specific status code than the
 * default conversion from VkResult. This can be used to provide more granular error information based on the context of
 * the failure.
 * @param hr The VkResult value returned by a Vulkan function call that indicates the result of the operation.
 * @param sl The source location information, which defaults to the current location where wis::detail::make_result is
 * called. This is used to extract the function name for the error message.
 ...
 */
template <func_pair func, wis::fixed_string message>
WIS_CONSTEXPR23 WisResult
make_result(WisStatus status, VkResult hr, wis::source_location sl = wis::source_location::current()) noexcept
{
    static const auto str = wis::detail::make_error_string<message, func>(sl);
