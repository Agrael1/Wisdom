...
bool wis::ImplVKRaytracing::GetExtensionInfo(
...
) noexcept
{
    ...
    structure_map[VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_RAY_TRACING_PIPELINE_FEATURES_KHR] = sizeof(
        VkPhysicalDeviceRayTracingPipelineFeaturesKHR
    );
    property_map[VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_RAY_TRACING_PIPELINE_PROPERTIES_KHR] = sizeof(
        VkPhysicalDeviceRayTracingPipelinePropertiesKHR
    );

    ext_name_set.emplace(VK_KHR_ACCELERATION_STRUCTURE_EXTENSION_NAME);
    structure_map[VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_ACCELERATION_STRUCTURE_FEATURES_KHR] = sizeof(
        VkPhysicalDeviceAccelerationStructureFeaturesKHR
    );
    property_map[VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_ACCELERATION_STRUCTURE_PROPERTIES_KHR] = sizeof(
    ...
    structure_map[VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_RAY_QUERY_FEATURES_KHR] = sizeof(
        VkPhysicalDeviceRayQueryFeaturesKHR
    );
    return true;
}

wis::Result wis::ImplVKRaytracing::Init(
...
) noexcept
{
    ...
    auto props = reinterpret_cast<const VkPhysicalDeviceRayTracingPipelinePropertiesKHR*>(
        property_map.at(VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_RAY_TRACING_PIPELINE_PROPERTIES_KHR)
    );

    compressed_handle_size = props->shaderGroupHandleSize;
    sbt_info = {
        wis::detail::aligned_size(compressed_handle_size, props->shaderGroupHandleAlignment),
...
wis::ASAllocationInfo wis::ImplVKRaytracing::GetTopLevelASSize(const wis::TopLevelASBuildDesc& tlas_desc) const noexcept
{
    ...
    VkAccelerationStructureBuildGeometryInfoKHR build_info{
        ...
        .mode = tlas_desc.update ? VK_BUILD_ACCELERATION_STRUCTURE_MODE_UPDATE_KHR
...
wis::ASAllocationInfo wis::ImplVKRaytracing::GetBottomLevelASSize(
...
) const noexcept
{
    ...
    wis::detail::limited_allocator<
        uint8_t,
        initial_geometry_guess * (sizeof(uint32_t) + sizeof(VkAccelerationStructureGeometryKHR*))>
    ...
    VkAccelerationStructureBuildGeometryInfoKHR build_info{
        ...
        .mode = blas_desc.update ? VK_BUILD_ACCELERATION_STRUCTURE_MODE_UPDATE_KHR
                                 : VK_BUILD_ACCELERATION_STRUCTURE_MODE_BUILD_KHR,
        .geometryCount = blas_desc.geometry_count,
        .ppGeometries = direct ? reinterpret_cast<const VkAccelerationStructureGeometryKHR* const*>(data)
                               : reinterpret_cast<const VkAccelerationStructureGeometryKHR* const*>(
                                     blas_desc.geometry_indirect
                                 ),
    };
    VkAccelerationStructureBuildSizesInfoKHR build_sizes_info{
        .sType = VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_BUILD_SIZES_INFO_KHR,
...
wis::VKRaytracingPipeline wis::ImplVKRaytracing::CreateRaytracingPipeline(
    wis::Result& result,
    const wis::VKRaytracingPipeineDesc& rt_pipeline_desc
) const noexcept
{
    ...
    pipe_i.shader_identifiers = wis::detail::make_unique_for_overwrite<uint8_t[]>(
        handle_size
    ); // expanded size for all shader groups
    ...
    std::span<VkPipelineShaderStageCreateInfo> stages_span{
        reinterpret_cast<VkPipelineShaderStageCreateInfo*>(stages.get()),
        rt_pipeline_desc.export_count
    };
    std::span<VkRayTracingShaderGroupCreateInfoKHR> raygen_span{
        reinterpret_cast<VkRayTracingShaderGroupCreateInfoKHR*>(stages_span.data() + rt_pipeline_desc.export_count),
        raygen_count
    };
    std::span<VkRayTracingShaderGroupCreateInfoKHR> miss_span{raygen_span.data() + raygen_count, miss_count};
    std::span<VkRayTracingShaderGroupCreateInfoKHR> hit_group_span{
        miss_span.data() + miss_count,
        rt_pipeline_desc.hit_group_count
    };
    std::span<VkRayTracingShaderGroupCreateInfoKHR> callable_span{
        hit_group_span.data() + rt_pipeline_desc.hit_group_count,
        callable_count
    };
    ...
    for (uint32_t i = 0; i < rt_pipeline_desc.hit_group_count; ++i) {
        ...
        hit_group_span[i] = {
            ...
            .closestHitShader = hg.closest_hit_export_index == UINT32_MAX ? VK_SHADER_UNUSED_KHR
                                                                          : hg.closest_hit_export_index,
            .anyHitShader = hg.any_hit_export_index == UINT32_MAX ? VK_SHADER_UNUSED_KHR : hg.any_hit_export_index,
            .intersectionShader = hg.intersection_export_index == UINT32_MAX ? VK_SHADER_UNUSED_KHR
    ...
    auto vr = table.vkCreateRayTracingPipelinesKHR(
        device.get(),
        VK_NULL_HANDLE,
        VK_NULL_HANDLE,
        1,
        &pipeline_info,
        nullptr,
        pipe_i.state_object.put(device, device.table().vkDestroyPipeline)
    );
    if (!wis::succeeded(vr)) {
...
void wis::ImplVKRaytracing::BuildBottomLevelAS(
    wis::VKCommandListView cmd_buffer,
    const wis::VKBottomLevelASBuildDesc& blas_desc,
    wis::VKAccelerationStructureView dst_acceleration_structure,
    uint64_t scratch_buffer_gpu_address,
    wis::VKAccelerationStructureView src_acceleration_structure
) const noexcept
{
    // one is ppGeometries, the other is maxPrimitiveCount
    uint32_t direct = bool(blas_desc.geometry_array);
...
void wis::ImplVKRaytracing::BuildTopLevelAS(
    wis::VKCommandListView cmd_buffer,
...
wis::RaytracingConstants wis::ImplVKRaytracing::GetRaytracingConstants() const noexcept
{
    return RaytracingConstants{.max_recursion_depth = max_recursion_depth};
}

void wis::ImplVKRaytracing::CopyAccelerationStructure(
