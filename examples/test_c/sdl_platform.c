...
WisSurface CreateSurface(const SDLPlatform* platform, SDL_Window* window)
{
    ...
    case SDL_PLATFORM_EXTENSION_X11: {
#ifdef WIS_PLATFORM_XLIB_PRESENT
        void* xdisplay = (void*)SDL_GetPointerProperty(SDL_GetWindowProperties(window), SDL_PROP_WINDOW_X11_DISPLAY_POINTER, NULL);
        ...
        if (xdisplay && xwindow) {
            ...
            WisSurface surface = { 0 };
            WisResult  result  = wisXlibExtensionCreateSurface((WisXlibExtension*)platform->platform_extension, &desc, &surface);
