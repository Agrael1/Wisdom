 ...
 * - `WISDOM_USE_AGILITY_SDK=OFF` download and build with Agility SDK instead of Windows SDK, this allows using latest
 * DirectX 12 features on older Windows versions, but requires additional setup and dependencies. Default is `OFF`,
 * which uses Windows SDK that comes with the system and DirectX-Headers.
