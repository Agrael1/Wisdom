...
WIS_EXPORT class VKCommandList : public QueryInternal<VKCommandList>
{
public:
    VKCommandList() = default;
    explicit VKCommandList(wis::shared_handle<vk::CommandPool> allocator, vk::CommandBuffer command_list) noexcept
    ...
    void CopyTexture(VKBufferView source, VKTextureView destination, TextureRange range) noexcept
    {
        vk::BufferImageCopy img_copy{
            .bufferOffset = 0u, // bufferOffset
            .bufferRowLength = 0u, // bufferRowLength
            .bufferImageHeight = 0u, // bufferImageHeight
            .imageSubresource = vk::ImageSubresourceLayers{ aspect_flags(destination.format), range.mip, range.array_layer, 1u }, // imageSubresource
            .imageOffset = { 0, 0, 0 }, // imageOffset
            .imageExtent = vk::Extent3D{ range.extent.width, range.extent.height, range.extent.depth } // imageExtent
        };
    ...
    void CopyTextureToBuffer(VKTextureView source, VKBufferView destination, TextureRange range) noexcept
    {
        vk::BufferImageCopy img_copy{
            ...
            .bufferRowLength = 0u, // bufferRowLength
            .bufferImageHeight = 0u, // bufferImageHeight
            .imageSubresource = vk::ImageSubresourceLayers{ aspect_flags(source.format), range.mip, range.array_layer, 1u }, // imageSubresource
            .imageOffset = { 0, 0, 0 }, // imageOffset
            .imageExtent = vk::Extent3D{ range.extent.width, range.extent.height, range.extent.depth } // imageExtent
        };
        command_list.copyImageToBuffer(source.image, vk::ImageLayout::eTransferSrcOptimal, destination, img_copy);
    }
