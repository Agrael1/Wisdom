...
public:
    ...
    operator DX12MemoryView() const noexcept { return {allocator.get(), allocation.get()}; }
    [[nodiscard]] bool operator==(std::nullptr_t) const noexcept { return !allocation; }
    [[nodiscard]] bool operator==(const ImplDX12Memory& other) const noexcept
    {
...
public:
     ...
     * */
    inline uint64_t GetBlockOffset() const noexcept { return wis::ImplDX12Memory::GetBlockOffset(); }
};
