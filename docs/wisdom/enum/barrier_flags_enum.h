 ...
 * - `WisBarrierFlagsDepthResource = (1 << 0)`: Resource is a depth resource. This flag @wis_must be set for all depth
 * resources to make transitions on them.
 * - `WisBarrierFlagsStencilResource = (1 << 1)`: Resource is a stencil resource. This flag @wis_must be set for all
 * stencil resources to make transitions on them.
 * - `WisBarrierFlagsWholeRange = (1 << 2)`: Transition whole resource. If not set, the transition is applied only to
