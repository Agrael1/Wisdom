...
public:
    ...
    [[nodiscard]] static std::string LayersString() noexcept
    {
        ...
        for (const auto& [name, _] : GetLayers()) {
            wis::format_to(std::back_inserter(debug_str1),
                           "\t{}\n",
                           name);
        }
...
private:
    FactoryData(bool blayers = true, bool bextensions = true) noexcept
    {
        if (bextensions)
            LoadExtensions();
        if (blayers)
    ...
    void LoadExtensions() noexcept
    {
        ...
        for (auto& i : extensions)
            m_extensions.emplace(i.extensionName, i);
    }
    void LoadLayers() noexcept
    {
        ...
        for (auto& i : layers)
            m_layers.emplace(i.layerName, i);
    }

    std::unordered_map<std::string, vk::ExtensionProperties, wis::string_hash> m_extensions;
    std::unordered_map<std::string, vk::LayerProperties, wis::string_hash> m_layers;
};
...
wis::VKFactory::VKFactory(const ApplicationInfo& app_info)noexcept
{
    ...
    vk::ApplicationInfo info{
        .pApplicationName = app_info.application_name,
        .applicationVersion = VK_MAKE_API_VERSION(0, app_info.app_version.major, app_info.app_version.minor, app_info.app_version.patch),
        .pEngineName = app_info.engine_name,
        .engineVersion = VK_MAKE_API_VERSION(0, app_info.engine_version.major, app_info.engine_version.minor, app_info.engine_version.patch),
        .apiVersion = version
    };
    ...
    constexpr static vk::DebugUtilsMessengerCreateInfoEXT create_instance_debug{
        .messageSeverity = vk::DebugUtilsMessageSeverityFlagsEXT(VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT),
        .messageType = vk::DebugUtilsMessageTypeFlagsEXT(VK_DEBUG_UTILS_MESSAGE_TYPE_GENERAL_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_TYPE_VALIDATION_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_TYPE_PERFORMANCE_BIT_EXT),
        .pfnUserCallback = VKFactory::DebugCallback
    };
    ...
    DynamicLoader::loader = vk::DispatchLoaderDynamic{ instance, vkGetInstanceProcAddr };
