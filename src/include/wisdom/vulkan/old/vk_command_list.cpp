...
void wis::ImplVKCommandList::CopyBuffer(
    VKBufferView source,
    VKBufferView destination,
...
void wis::ImplVKCommandList::CopyBufferToTexture(
    VKBufferView src_buffer,
    VKTextureView dest_texture,
...
) const noexcept
{
    ...
    for (size_t i = 0; i < region_count; i++) {
        ...
        copies[i] = VkBufferImageCopy2{
            ...
            .imageOffset =
                {int(region.texture.offset.width),
                 int(region.texture.offset.height),
                 int(region.texture.offset.depth_or_layers)},
...
void wis::ImplVKCommandList::CopyTextureToBuffer(
    VKTextureView src_texture,
    VKBufferView dst_buffer,
    const wis::BufferTextureCopyRegion* regions,
    uint32_t region_count
) const noexcept
{
...
void wis::ImplVKCommandList::CopyTexture(
    VKTextureView src_texture,
    VKTextureView dst_texture,
...
) const noexcept
{
    ...
    for (size_t i = 0; i < region_count; i++) {
        ...
        copies[i] = VkImageCopy2{
            ...
            .srcOffset =
                {int(region.src.offset.width), int(region.src.offset.height), int(region.src.offset.depth_or_layers)},
            .dstSubresource =
                {
            ...
            .dstOffset =
                {int(region.dst.offset.width), int(region.dst.offset.height), int(region.dst.offset.depth_or_layers)},
...
inline VkBufferMemoryBarrier2 to_vk(wis::BufferBarrier barrier, VkBuffer buffer) noexcept
{
    return VkBufferMemoryBarrier2{
        ...
        .pNext = nullptr,
        .srcStageMask = wis::detail::VKConvert(barrier.sync_before),
        .srcAccessMask = wis::detail::VKConvert(barrier.access_before),
        .dstStageMask = wis::detail::VKConvert(barrier.sync_after),
        .dstAccessMask = wis::detail::VKConvert(barrier.access_after),
...
inline VkImageMemoryBarrier2 to_vk(wis::TextureBarrier barrier, VkImage texture, VkFormat format) noexcept
{
    ...
    bool zero_range = subresource.base_array_layer == 0 && subresource.base_mip_level == 0 &&
                      subresource.layer_count == 0 && subresource.level_count == 0;
    ...
    return VkImageMemoryBarrier2{
        ...
        .dstStageMask = wis::detail::VKConvert(barrier.sync_after),
        .dstAccessMask = wis::detail::VKConvert(barrier.access_after),
        .oldLayout = wis::detail::VKConvert(barrier.state_before),
        .newLayout = wis::detail::VKConvert(barrier.state_after),
        .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
        .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
        .image = texture,
        .subresourceRange = {
...
void wis::ImplVKCommandList::TextureBarriers(const wis::VKTextureBarrier2* barriers, uint32_t barrier_count) noexcept
{
    ...
    for (size_t i = 0; i < barrier_count; i++) {
        data[i] = detail::to_vk(
            barriers[i].barrier,
            std::get<0>(barriers[i].texture),
            std::get<1>(barriers[i].texture)
        );
    }
    VkDependencyInfo depinfo{
        .sType = VK_STRUCTURE_TYPE_DEPENDENCY_INFO,
...
void wis::ImplVKCommandList::BeginRenderPass(const wis::VKRenderPassDesc& pass_desc) noexcept
{
    ...
    for (size_t i = 0; i < pass_desc.target_count; i++) {
        ...
        data[i] = VkRenderingAttachmentInfo{
            ...
            .loadOp = wis::detail::VKConvert(target.load_op),
            .storeOp = wis::detail::VKConvert(target.store_op),
        };
        if (data[i].loadOp == VK_ATTACHMENT_LOAD_OP_CLEAR) {
            data[i].clearValue = {
                .color = {
                    .float32{target.clear_value[0], target.clear_value[1], target.clear_value[2], target.clear_value[3]}
                }
            };
        }
    }
    ...
    if (ds_selector & DSSelect::Depth) {
        d_info = {
            ...
            .loadOp = wis::detail::VKConvert(pass_desc.depth_stencil->load_op_depth),
            .storeOp = wis::detail::VKConvert(pass_desc.depth_stencil->store_op_depth),
        };
    ...
    if (ds_selector & DSSelect::Stencil) {
        s_info = {
            ...
            .loadOp = wis::detail::VKConvert(pass_desc.depth_stencil->load_op_stencil),
            .storeOp = wis::detail::VKConvert(pass_desc.depth_stencil->store_op_stencil),
        };
    ...
    VkRenderingInfo info{
        ...
        .pNext = nullptr,
        .flags = wis::detail::VKConvert(pass_desc.flags),
        .renderArea =
            {
                .offset = {0, 0},
...
{
    device.table().vkCmdSetPrimitiveTopology(command_list, wis::detail::VKConvert(topology));
}
...
void wis::ImplVKCommandList::IASetVertexBuffers(
...
{
    device.table().vkCmdBindIndexBuffer(command_list, std::get<0>(buffer), offset, wis::detail::VKConvert(type));
}
void wis::ImplVKCommandList::IASetIndexBuffer2(
...
{
    device.table().vkCmdBindIndexBuffer2(command_list, std::get<0>(buffer), offset, size, wis::detail::VKConvert(type));
}

void wis::ImplVKCommandList::DrawIndexedInstanced(
...
void wis::ImplVKCommandList::DrawInstanced(
    uint32_t vertex_count_per_instance,
    uint32_t instance_count,
    uint32_t base_vertex,
    uint32_t start_instance
...
void wis::ImplVKCommandList::SetPushConstants(
    const void* data,
...
{
    device.table().vkCmdPushConstants(
        command_list,
        pipeline_layout,
        wis::detail::VKConvert(stage),
        offset_4bytes * 4,
        size_4bytes * 4,
        data
    );
...
void wis::ImplVKCommandList::VKPushDescriptor(
    wis::DescriptorType type,
    uint32_t binding,
    wis::VKBufferView view,
    uint32_t offset,
...
) noexcept
{
    ...
    VkWriteDescriptorSet descriptor{
        ...
        .dstArrayElement = 0,
        .descriptorCount = 1,
        .descriptorType = wis::detail::VKConvert(type),
        .pBufferInfo = &buffer_info
    };
    device.table().vkCmdPushDescriptorSet(
        command_list,
        binding_point,
        pipeline_layout,
...
void wis::ImplVKCommandList::VKSetDescriptorStorage(
    wis::VKDescriptorStorageView desc_storage,
    VkPipelineBindPoint binding_point
) noexcept
{
    auto& set_span = std::get<0>(desc_storage);
    device.table().vkCmdBindDescriptorSets(
        command_list,
        binding_point,
        pipeline_layout,
