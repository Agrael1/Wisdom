...
wis::Result wis::detail::VKSwapChainCreateInfo::AcquireNextIndex() const noexcept
{
    ...
    auto result = dtable.vkAcquireNextImageKHR(
        device.get(),
        swapchain,
        std::numeric_limits<uint64_t>::max(),
        image_ready_semaphores[acquire_index],
        nullptr,
        &present_index
    );
    if (!wis::succeeded(result)) {
    ...
    return wis::succeeded(result) ? wis::success
                                  : wis::make_result<wis::Func<wis::FuncD()>(), "vkQueueSubmit failed">(result);
}
...
void wis::detail::VKSwapChainCreateInfo::ReleaseSemaphores() noexcept
{
    ...
    dtable.vkWaitForFences(
        device.get(),
        2,
        reinterpret_cast<VkFence*>(fences.data()),
        VK_TRUE,
        std::numeric_limits<uint64_t>::max()
    );
...
wis::Result wis::ImplVKSwapChain::WaitForPresent(uint64_t timeout_ns) const noexcept
{
    auto& dtable = device.table();
    if (dtable.vkWaitForPresent2KHR) {
        ...
        return wis::succeeded(res) ? wis::success
                                   : wis::make_result<wis::Func<wis::FuncD()>(), "vkWaitForPresent2KHR failed">(res);
    }
    ...
    return wis::succeeded(res) ? wis::success
                               : wis::make_result<wis::Func<wis::FuncD()>(), "vkWaitForPresentKHR failed">(res);
}

wis::Result wis::ImplVKSwapChain::VKRecreateSwapchain(uint32_t width, uint32_t height, void* pNext) noexcept
{
    auto& dtable = device.table();
...
wis::Result wis::ImplVKSwapChain::VKPresent(void* pNext) const noexcept
{
    auto& dtable = device.table();
