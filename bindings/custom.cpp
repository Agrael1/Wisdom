...
template<typename T>
struct GetExtSize {
    consteval std::size_t operator()() const noexcept
    {
...
static inline uint8_t* wisdom_alloc(std::size_t size, std::size_t align)
{
...
{
    std::size_t size = 0;
    std::size_t all_exts = 0;
    std::size_t first_al = 1;
    for (auto&& ext : exts) {
        if (ext.extension_id == 0) {
            continue;
        }
        std::size_t align = DX12BridgeSelector<PreType, GetExtAlign>(static_cast<EnumT>(ext.extension_id));
        if (all_exts == 0) {
            first_al = align;
        }
    ...
    std::size_t array_size = all_exts * sizeof(EnumT); // can be 0
    std::size_t cap_size = sizeof(uint32_t);

    std::size_t base_size = wis::detail::aligned_size(sizeof(PreType), alignof(uint32_t)) + cap_size + array_size;
    std::size_t overall_size = wis::detail::aligned_size(base_size, first_al) + size;
...
static inline ExtLayout<PreType, EnumT> from_stream_dx12(uint8_t* stream) noexcept
{
    ...
    uint8_t* ext_count = reinterpret_cast<uint8_t*>(wis::detail::aligned_size(std::size_t(stream) + sizeof(PreType), alignof(uint32_t)));
        ...
        for (std::size_t i = 0; i < extension_count; i++) {
            if (extensions[i].extension_id != 0) {
                ...
                *ext_ids = id;
                ...
                std::size_t align = DX12FactoryExtensionBridge<GetExtAlign>(id);
                ext_ptr = reinterpret_cast<uint8_t*>(wis::detail::aligned_size((std::size_t)ext_ptr, align));
...
extern "C" void DX12FactoryDestroy(DX12Factory self)
{
    ...
    for (auto eid : memory.exts) {
        ext_ptr = reinterpret_cast<uint8_t*>(wis::detail::aligned_size((std::size_t)ext_ptr, DX12FactoryExtensionBridge<GetExtAlign>(eid)));
        ...
        for (std::size_t i = 0; i < extension_count; i++) {
            if (extensions[i].extension_id != 0) {
                ...
                *ext_ids = id;
                ...
                std::size_t align = DX12DeviceExtensionBridge<GetExtAlign>(id);
                ext_ptr = reinterpret_cast<uint8_t*>(wis::detail::aligned_size((std::size_t)ext_ptr, align));
...
extern "C" void DX12DeviceDestroy(DX12Device self)
{
    ...
    uint8_t* ext_ptr = memory.ext_memory;
    for (auto eid : memory.exts) {
        ext_ptr = reinterpret_cast<uint8_t*>(wis::detail::aligned_size((std::size_t)ext_ptr, DX12DeviceExtensionBridge<GetExtAlign>(eid)));
...
static inline ExtLayout<PreType, EnumT> wisdom_alloc_ext_vk(std::span<ExtT> exts) noexcept
{
    ...
    for (auto&& ext : exts) {
        ...
        std::size_t align = VKBridgeSelector<PreType, GetExtAlign>(static_cast<EnumT>(ext.extension_id));
        if (all_exts == 0) {
            first_al = align;
        }
        ...
        for (std::size_t i = 0; i < extension_count; i++) {
            if (extensions[i].extension_id != 0) {
                ...
                *ext_ids = id;
                ...
                std::size_t align = VKFactoryExtensionBridge<GetExtAlign>(id);
                ext_ptr = reinterpret_cast<uint8_t*>(wis::detail::aligned_size((std::size_t)ext_ptr, align));
...
extern "C" void VKFactoryDestroy(VKFactory self)
{
    ...
    uint8_t* ext_ptr = memory.ext_memory;
    for (auto eid : memory.exts) {
        ext_ptr = reinterpret_cast<uint8_t*>(wis::detail::aligned_size((std::size_t)ext_ptr, VKFactoryExtensionBridge<GetExtAlign>(eid)));
        ...
        for (std::size_t i = 0; i < extension_count; i++) {
            if (extensions[i].extension_id != 0) {
                ...
                *ext_ids = id;
                ...
                std::size_t align = VKDeviceExtensionBridge<GetExtAlign>(id);
                ext_ptr = reinterpret_cast<uint8_t*>(wis::detail::aligned_size((std::size_t)ext_ptr, align));
...
extern "C" void VKDeviceDestroy(VKDevice self)
{
    ...
    uint8_t* ext_ptr = memory.ext_memory;
    for (auto eid : memory.exts) {
        ext_ptr = reinterpret_cast<uint8_t*>(wis::detail::aligned_size((std::size_t)ext_ptr, VKDeviceExtensionBridge<GetExtAlign>(eid)));
