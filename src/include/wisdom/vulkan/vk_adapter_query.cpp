...
inline WisResult VKInitResourceAllocator(
    VkDevice device,
    VkPhysicalDevice adapter,
...
)
{
    ...
    // Initialize queues
    auto queue_info = VKGetQueueResidencyInfo(atable, adapter, requirements, device_ext1.features, res);
    if (res.status != WisStatusOk) {
        return res;
    }
    ...
    static_assert(
        alignof(wis::detail::VKDeviceControlBlock) <= __STDCPP_DEFAULT_NEW_ALIGNMENT__,
        "VKDeviceControlBlock must be well aligned to safely use operator new for allocation"
    );
