...
public:
    ...
    void* MapRaw() const noexcept { return Map(); }
    void Unmap() const noexcept { resource->Unmap(0, nullptr); }
...
public:
    ...
    operator DX12RenderTargetView() const noexcept { return handle; }
};
...
public:
    ...
    operator bool() const noexcept { return bool(heap); }
    operator DX12SamplerView() const noexcept { return heap->GetCPUDescriptorHandleForHeapStart(); }
};
...
public:
    ...
    operator bool() const noexcept { return bool(heap); }
    operator DX12ShaderResourceView() const noexcept { return heap->GetCPUDescriptorHandleForHeapStart(); }
};
...
public:
     ...
     * */
    inline void* MapRaw() const noexcept { return wis::ImplDX12Buffer::MapRaw(); }
    /**
     * @brief Unmaps the buffer memory from CPU address space.
     * */
    inline void Unmap() const noexcept { wis::ImplDX12Buffer::Unmap(); }
    /**
     * @brief Returns the address of the resource in GPU memory.
     * @return The address of the resource in GPU memory.
     * */
    inline uint64_t GetGPUAddress() const noexcept { return wis::ImplDX12Buffer::GetGPUAddress(); }
};
