...
void wis::ImplDX12CommandList::CopyBuffer(
...
void wis::ImplDX12CommandList::CopyBufferToTexture(
...
) const noexcept
{
    ...
    for (uint32_t i = 0; i < region_count; i++) {
        ...
        uint32_t dest_subresource = D3D12CalcSubresource(
            region.texture.mip,
            region.texture.array_layer,
            0u,
        ...
        D3D12_PROPERTY_LAYOUT_FORMAT_TABLE::CalculateMinimumRowMajorRowPitch(
            DX12Convert(region.texture.format),
            region.texture.size.width,
            row_pitch
        );
        ...
        list->CopyTextureRegion(
            &dst,
            region.texture.offset.width,
            region.texture.offset.height,
            region.texture.offset.depth_or_layers,
            &src,
            nullptr
        );
    }
...
void wis::ImplDX12CommandList::CopyTexture(
...
) const noexcept
{
    ...
    for (uint32_t i = 0; i < region_count; i++) {
        ...
        uint64_t row_size = 0;
        uint64_t required_size = 0;
        uint32_t src_subresource = D3D12CalcSubresource(
            region.src.mip,
        ...
        uint32_t dst_subresource = D3D12CalcSubresource(
            region.dst.mip,
            region.dst.array_layer,
            0u,
            dst_desc.MipLevels,
            dst_desc.DepthOrArraySize
        );
        D3D12_TEXTURE_COPY_LOCATION dst{
        ...
        list->CopyTextureRegion(
            &dst,
            region.dst.offset.width,
            region.dst.offset.depth_or_layers,
            region.dst.offset.depth_or_layers,
            &src,
            &box
        );
...
void wis::ImplDX12CommandList::CopyTextureToBuffer(
...
wis::Result wis::ImplDX12CommandList::Reset(wis::DX12PipelineView pipeline) noexcept
{
    ...
    closed = false;
    return wis::succeeded(hr) ? wis::success
                              : wis::make_result<wis::Func<wis::FuncD()>(), "Reset failed (command list)">(hr);
}
...
void wis::ImplDX12CommandList::BufferBarrier(wis::BufferBarrier barrier, DX12BufferView buffer) noexcept
{
    ...
    D3D12_BARRIER_GROUP bg{.Type = D3D12_BARRIER_TYPE_BUFFER, .NumBarriers = 1, .pBufferBarriers = &bb};
    list->Barrier(1, &bg);
}
...
void wis::ImplDX12CommandList::BufferBarriers(const wis::DX12BufferBarrier2* barriers, uint32_t barrier_count) noexcept
{
    ...
    D3D12_BARRIER_GROUP bg{.Type = D3D12_BARRIER_TYPE_BUFFER, .NumBarriers = barrier_count, .pBufferBarriers = data};
    list->Barrier(1, &bg);
}
...
void wis::ImplDX12CommandList::TextureBarrier(wis::TextureBarrier barrier, DX12TextureView texture) noexcept
{
    ...
    D3D12_BARRIER_GROUP bg{.Type = D3D12_BARRIER_TYPE_TEXTURE, .NumBarriers = 1, .pTextureBarriers = &tb};
    list->Barrier(1, &bg);
}
void wis::ImplDX12CommandList::TextureBarriers(const wis::DX12TextureBarrier2* barrier, uint32_t barrier_count) noexcept
{
    ...
    D3D12_BARRIER_GROUP bg{.Type = D3D12_BARRIER_TYPE_TEXTURE, .NumBarriers = barrier_count, .pTextureBarriers = data};
    list->Barrier(1, &bg);
}
...
void wis::ImplDX12CommandList::BeginRenderPass(const wis::DX12RenderPassDesc& pass_desc) noexcept
{
    ...
    for (size_t i = 0; i < pass_desc.target_count; i++) {
        ...
        if (data[i].BeginningAccess.Type == D3D12_RENDER_PASS_BEGINNING_ACCESS_TYPE_CLEAR) {
            std::copy(
                std::begin(target.clear_value),
                std::end(target.clear_value),
                std::begin(data[i].BeginningAccess.Clear.ClearValue.Color)
            );
        }
    }
    ...
    if (ds_selector != DSSelect::None) {
        depth_stencil = {
            ...
            .DepthBeginningAccess =
                {
                    .Type = ds_selector & DSSelect::Depth
                                ? wis::detail::DX12Convert(pass_desc.depth_stencil->load_op_depth)
            ...
            .StencilBeginningAccess =
                {.Type = ds_selector & DSSelect::Stencil
                             ? wis::detail::DX12Convert(pass_desc.depth_stencil->load_op_stencil)
                             : D3D12_RENDER_PASS_BEGINNING_ACCESS_TYPE_NO_ACCESS,
                 .Clear = {.ClearValue{.DepthStencil{
                     .Depth = pass_desc.depth_stencil->clear_depth,
                     .Stencil = pass_desc.depth_stencil->clear_stencil
            ...
            .DepthEndingAccess =
                {
                    .Type = ds_selector & DSSelect::Depth
                                ? wis::detail::DX12Convert(pass_desc.depth_stencil->store_op_depth)
                                : D3D12_RENDER_PASS_ENDING_ACCESS_TYPE_NO_ACCESS,
                },
            .StencilEndingAccess = {
                .Type = ds_selector & DSSelect::Stencil
                            ? wis::detail::DX12Convert(pass_desc.depth_stencil->store_op_stencil)
                            : D3D12_RENDER_PASS_ENDING_ACCESS_TYPE_NO_ACCESS,
            }
        };
    }
    ...
    list->BeginRenderPass(
        pass_desc.target_count,
        data,
        (ds_selector != DSSelect::None) ? &depth_stencil : nullptr,
        wis::detail::DX12Convert(pass_desc.flags)
    );
...
void wis::ImplDX12CommandList::IASetVertexBuffers(
...
void wis::ImplDX12CommandList::IASetIndexBuffer(
...
void wis::ImplDX12CommandList::IASetIndexBuffer2(
    wis::DX12BufferView buffer,
    wis::IndexType type,
    uint32_t size,
    uint64_t offset
...
void wis::ImplDX12CommandList::DrawIndexedInstanced(
...
void wis::ImplDX12CommandList::DrawInstanced(
    uint32_t vertex_count_per_instance,
    uint32_t instance_count,
    uint32_t base_vertex,
    uint32_t start_instance
...
void wis::ImplDX12CommandList::SetPushConstants(
    const void* data,
...
void wis::ImplDX12CommandList::SetComputePushConstants(
    const void* data,
    uint32_t size_4bytes,
    uint32_t offset_4bytes
) noexcept
...
void wis::ImplDX12CommandList::PushDescriptor(
...
void wis::ImplDX12CommandList::PushDescriptorCompute(
    wis::DescriptorType type,
    uint32_t binding,
    wis::DX12BufferView view,
    uint32_t offset
