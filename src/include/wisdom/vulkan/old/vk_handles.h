...
public:
    ...
    explicit SharedDevice(
        VkDevice device,
        std::unique_ptr<VKMainDevice> device_table,
...
public:
    ...
    auto& gtable() const noexcept { return *m_global_table; }
...
public:
    ...
    explicit SharedInstance(
        VkInstance device,
        PFN_vkDestroyInstance deleter,
        std::unique_ptr<VKMainInstance> instance_table,
        VKMainGlobal* global_table
    ) noexcept
...
public:
    auto& table() const noexcept { return *m_instance_table; }
    auto& gtable() const noexcept { return *m_global_table; }
    template <typename PFN>
...
template <typename HandleType>
class managed_handle_ex
    : public managed_handle_base<HandleType, managed_header_ex<HandleType>, managed_handle_ex<HandleType>>
{
public:
    managed_handle_ex() = default;
    using managed_handle_base<HandleType, managed_header_ex<HandleType>, managed_handle_ex<HandleType>>::
        managed_handle_base;
};
...
public:
    ...
    [[nodiscard]] const auto& device() const noexcept { return header(); }
...
{
    using wis::shared_handle_base<VkSurfaceKHR, wis::managed_header_ex<VkSurfaceKHR>, SharedSurface>::
        shared_handle_base;
};
} // namespace wis
