...
wis::detail::fixed_allocation<const char*> wis::ImplVKFactory::FoundExtensions(
    wis::Result& result,
...
) noexcept
{
    ...
    wis::detail::fixed_allocation<const char*> found_extension = wis::detail::make_fixed_allocation<const char*>(
        exts_set.size()
    );
    if (!found_extension) {
        result = wis::make_result<wis::Func<wis::FuncD()>(), "Not enough memory for extensions">(
...
wis::detail::fixed_allocation<const char*> wis::ImplVKFactory::FoundLayers(
    wis::Result& result,
...
) noexcept
{
    ...
    if (!found_layers) {
        result = wis::make_result<wis::Func<wis::FuncD()>(), "Not enough memory for layers">(
            VK_ERROR_OUT_OF_HOST_MEMORY
        );
        return found_layers;
    }
...
) noexcept
{
    ...
    if (!table) {
        result = wis::make_result<wis::Func<wis::FuncD()>(), "Failed to create instance table">(
            VK_ERROR_OUT_OF_HOST_MEMORY
        );
        return out_factory;
    }
...
//--------------------------------------------------------------------------------------------------
wis::VKFactory wis::ImplVKCreateFactory(
    wis::Result& result,
    bool debug_layer,
...
) noexcept
{
    ...
    auto factory = detail::VKCreateFactoryWithExtensions(
        result,
        debug_layer,
        ext_alloc_raw,
        ext_alloc_size,
