...
public:
    ...
    operator DX12RaytracingPipelineView() noexcept { return {state_object.get()}; }
    const uint8_t* GetShaderIdentifiers() const noexcept { return shader_identifiers.get(); }
    operator bool() const noexcept { return state_object.get() != nullptr; }
};
} // namespace wis
