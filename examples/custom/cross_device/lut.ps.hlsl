...
float3 cscApplyLut3DTetra(const float3 color)
{
    ...
    float4 s1 = lut.Sample(sampler_point, blackf); // lower diagonal
    float4 s2 = lut.Sample(sampler_point, point_rgb);
    float4 s3 = lut.Sample(sampler_point, point_cmy);
    float4 s4 = lut.Sample(sampler_point, whitef); // upper diagonal
